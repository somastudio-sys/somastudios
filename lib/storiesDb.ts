...
export async function listStories(): Promise<RepurposedStory[]> {
  await ensureSchema();
  const sql = getSql();
...
export async function getStoryById(id: string): Promise<RepurposedStory | null> {
  await ensureSchema();
  const sql = getSql();
  const rows = await sql`
    SELECT id, created_at, title, genre, dream_id, body
...
export async function createStory(entry: {
  title: string;
  genre: string;
  dreamId?: string;
  body: string;
  ...
  await sql`
    INSERT INTO repurposed_stories (id, created_at, title, genre, dream_id, body)
    VALUES (
      ${story.id},
      ${story.createdAt},
      ${story.title},
...
export async function deleteStory(id: string): Promise<boolean> {
  await ensureSchema();
  const sql = getSql();
  const rows = await sql`DELETE FROM repurposed_stories WHERE id = ${id} RETURNING id`;
  return (rows as { id: string }[]).length > 0;
}

export async function migrateStories(stories: RepurposedStory[]): Promise<number> {
  await ensureSchema();
  const sql = getSql();
  let imported = 0;
